...
class EntityClassifier():
    ...
    def load_data_set_fromdb(self):
        ...
        result = collections.Counter(list(map(operator.itemgetter(1), datadb)))
        result = sorted(list(result.items()), key = operator.itemgetter(1))
        data = []
        for key, value in result:
            for content, entity in datadb:
                entity = six.text_type(entity)
        ...
        dataset = [data[x:x + randint(15, 35)] for x in range(0, len(data), 100)]
